...
class DySample(nn.Module):
    def __init__(self, in_channels, scale=2, style='lp', groups=4, dyscope=False):
        ...
        self.offset = nn.Conv2d(in_channels, out_channels, 1) # in，out, bias;
    ...
    def sample(self, x, offset):
        B, _, H, W = offset.shape # nchw
        offset = offset.view(B, 2, -1, H, W) # the second dimension represent the x and y offsets for each grid point
        coords_h = torch.arange(H) + 0.5
        coords_w = torch.arange(W) + 0.5 # represent x,y coordinates of the image grid. 0.5 to center the coord within each pixel
        coords = torch.stack(torch.meshgrid([coords_w, coords_h])
                             ).transpose(1, 2).unsqueeze(1).unsqueeze(0).type(x.dtype).to(x.device) # Creates a coordinate grid by stacking,transposing the dimensions, and adding batch and channel dimensions. 
        # result is (b,2,h,w,2)
        normalizer = torch.tensor([W, H], dtype=x.dtype, device=x.device).view(1, 2, 1, 1, 1)
        coords = 2 * (coords + offset) / normalizer - 1 # Calculates the sampling coordinates by adding the offsets to the original coordinates, and multi 2
